...
1. no return type - a constructor cannot return a result which means that we cannot signal an error during the object intitalization, the only way of doing it is to throw an exception from a constructor.
